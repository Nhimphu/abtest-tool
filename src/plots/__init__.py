...
def plot_bayesian_posterior(alpha_prior, beta_prior, users_a, conv_a, users_b, conv_b) -> "go.Figure":
...
def plot_confidence_intervals(users_a, conv_a, users_b, conv_b, alpha=0.05) -> "go.Figure":
    ...
    fig.add_trace(go.Scatter(x=[ci_b[0], ci_b[1]], y=[0,0], mode='lines', line=dict(width=10),
                             name=f'B CI {(1-alpha)*100:.0f}%', hovertemplate='B: %{x:.2%}<extra></extra>'))
    fig.update_layout(
...
def plot_power_curve(p1, alpha, power) -> "go.Figure":
...
def plot_bootstrap_distribution(users_a, conv_a, users_b, conv_b, iterations=5000) -> "go.Figure":
    ...
    fig.update_layout(title='Bootstrap Distribution of Δ (B−A)', xaxis_title='Difference in CR', yaxis_title='Probability', margin=dict(l=40, r=20, t=50, b=40))
    return fig


def plot_alpha_spending(alpha, looks) -> "go.Figure":
