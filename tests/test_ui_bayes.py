...
def test_bayesian_analysis_computes_probability():
    ...
    expected_prob = trapz(
        [expected_pb[i] * expected_cdf_a[i] for i in range(len(expected_x))], expected_x
    )
...
def test_ui_renders_bayes(monkeypatch):
    ...
    class DummyWindow:
        ...
        results_text = types.SimpleNamespace(
            setHtml=lambda html: called.setdefault("html", html)
        )
    ...
    monkeypatch.setattr(
        ...
        lambda name: types.SimpleNamespace(
            bayesian_analysis=lambda *a: (0.6, [], [], [])
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: types.SimpleNamespace(
            display_plot=lambda f: called.setdefault("fig", f)
        ),
    )
